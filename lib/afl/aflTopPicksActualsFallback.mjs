...
function playersMatch(a, b) {
  ...
  // Tolerate missing middle names / initials.
  const as = na.split(' ');
  const bs = nb.split(' ');
  if (as.length >= 2 && bs.length >= 2 && as[0] === bs[0] && as[as.length - 1] === bs[bs.length - 1]) {
    return true;
  }
  return false;
}
...
async function lookupEspnActual({ playerName, homeTeam, awayTeam, gameDate }, cache) {
  ...
  const event = events.find((ev) => {
    ...
    const names = competitors.map((c) => c?.team?.displayName || c?.team?.name || c?.team?.shortDisplayName || '');
...
/**
 * Resolve actual disposals for one Top Pick from Wheeloratings, then ESPN.
