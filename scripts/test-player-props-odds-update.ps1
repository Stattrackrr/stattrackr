...
try {
    $updateResponse = Invoke-RestMethod -Uri "$baseUrl/api/nba/player-props/update-odds" -Method POST -TimeoutSec 60
    if ($updateResponse.success) {
        Write-Host "  ✅ Update completed successfully" -ForegroundColor Green
        Write-Host "  Updated: $($updateResponse.updated) props" -ForegroundColor Green
        Write-Host "  Not found: $($updateResponse.notFound) props" -ForegroundColor Gray
        Write-Host "  Total: $($updateResponse.total) props" -ForegroundColor Gray
    } else {
