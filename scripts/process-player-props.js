...
async function processPlayerProps() {
          ...
          try {
            ...
            if (gamesWithStats.length > 0) {
              ...
              if (prop.opponent && prop.opponent !== 'ALL' && prop.opponent !== 'N/A' && prop.opponent !== '') {
                ...
                // Filter H2H games - FIXED to handle players who changed teams
  ...
  if (shouldMerge && cacheToMerge && Array.isArray(cacheToMerge) && cacheToMerge.length > 0) {
    ...
    // For split jobs, each part saves to its own cache key - no merging needed
    console.log(`[GitHub Actions] ✅ Saving ${finalProps.length} props to part-specific cache (no merging - dashboard will combine)`);
  } else {
  ...
  // When splitting (by props OR by games), use retry logic to handle race conditions
  const maxRetries = (propsSplit || gameSplit) ? 3 : 1;
  let savedSuccessfully = false;
  
  for (let retry = 0; retry < maxRetries && !savedSuccessfully; retry++) {
    try {
      // When splitting, each part writes to its own cache key - NO MERGING needed
      // Dashboard will read all 3 parts and combine them
      // No need to wait or read other parts - just save our own props
      ...
      // For split jobs, each part writes to its own cache key (no merging or clearing other parts)
      ...
      const isSplitJob = propsSplit || gameSplit;
      console.log(`[GitHub Actions] 💾 Saving ${isSplitJob ? 'merged' : 'new'} cache with key: ${cacheKey}${retry > 0 ? ` (retry ${retry})` : ''}`);
  ...
  if (!savedSuccessfully && !propsSplit && !gameSplit) {
