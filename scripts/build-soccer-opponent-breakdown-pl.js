...
const OUT_PATH = path.join(process.cwd(), 'data', 'soccer-opponent-breakdown-premier-league.json');
...
function getOpponentNumeric(match, focalTeamName, stat) {
...
function statAverageForTeam(matches, teamName, stat) {
  if (stat.id === 'goals') {
    if (matches.length === 0) return null;
    const total = matches.reduce((sum, match) => sum + goalsConceded(match, teamName), 0);
  ...
  for (const match of matches) {
    const row = findStat(match, stat.statName);
    if (!row) continue;
    const value = getOpponentNumeric(match, teamName, row);
    if (value == null) continue;
    sum += value;
    gamesCounted += 1;
  }

  if (gamesCounted === 0) return matches.length > 0 ? { perGame: null, gamesCounted: 0 } : null;
  return { perGame: sum / gamesCounted, gamesCounted };
}

function computeTeamDefensiveAverages(team, matches) {
  const byStat = {};
  for (const stat of DISPLAY_STATS) {
    byStat[stat.id] = statAverageForTeam(matches, team.name, stat);
  }
  return {
    name: team.name,
    href: team.href,
...
async function fetchTeamCurrentSeasonResults(team, seasonYear) {
  ...
  if (feedSign && countryId && participantId) {
    for (let page = 1; page <= MAX_SHOW_MORE_PAGES; page += 1) {
      ...
      const reachedOlderSeason = pageMatches.some(
        (match) => getSoccerSeasonYearFromKickoffUnix(match.kickoffUnix) < seasonYear
      );
...
async function main() {
  ...
  console.log(`[Soccer Breakdown PL] Building snapshot for ${roster.length} teams from Soccerway`);
  console.log(`[Soccer Breakdown PL] Using season ${seasonYear}/${String(seasonYear + 1).slice(-2)}`);
  ...
  console.log(`[Soccer Breakdown PL] Fetching stats for ${uniqueMatches.length} unique matches`);
  await runPool(
    uniqueMatches,
    async (match, index) => {
  ...
  const teams = validTeams
    .map((entry) => {
      ...
      return computeTeamDefensiveAverages(entry.team, hydratedMatches);
    })
    .filter(Boolean);

  const snapshotTeams = teams.map((team) => {
    const stats = DISPLAY_STATS.map((stat) => {
      const ranking = rankTeamsByStat(teams, stat);
      return {
        id: stat.id,
        label: stat.label,
        ...
        leagueAverage: leagueAverage(teams, stat),
  ...
  const payload = {
    ...
    source: 'build-soccer-opponent-breakdown-pl',
  ...
  console.log(`[Soccer Breakdown PL] Wrote ${OUT_PATH}`);
  console.log(`[Soccer Breakdown PL] Teams sampled: ${snapshotTeams.length}/${roster.length}`);
}

main().catch((error) => {
  console.error('[Soccer Breakdown PL] Failed:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
