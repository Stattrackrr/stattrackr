...
import {
  ...
  setWorldCupCache,
  wcCacheLog,
  loadWorldCupPlayerHistoryBundle,
} from '@/lib/worldCupCache';
import {
  applyWorldCupPlayerShotsToStatRows,
  buildWorldCupPlayerShotStatsByMatch,
  worldCupMatchIdsNeedingPlayerShotEvents,
  buildWorldCupPlayerShotsByMatch,
  isWorldCupShotOnTarget,
...
const WC2026 = {
  ...
  playerShots: (id: number | string) => `wc:player:shots:${id}:v2`,
...
async function resolveRequestedPlayerNameWithFallback(
  payload: Record<string, unknown>,
  playerId: string | null,
  playerNameParam: string | null
): Promise<string | null> {
  const direct = resolveRequestedPlayerName(payload, playerId, playerNameParam);
  if (direct) return direct;
  if (!playerId) return null;
  const nameIndex = await readWcCache<Record<string, number>>(WC2026.playerIdByName);
  if (nameIndex) {
    for (const [normalizedName, id] of Object.entries(nameIndex)) {
      if (String(id) === playerId && normalizedName.trim()) {
...
async function attachPlayerSidePanelData(
  ...
  if (playerVsPool && Array.isArray(playerVsPool.players) && (playerVsPool.players as unknown[]).length > 0) {
    ...
    const poolOppBreakdown = playerVsPool.opponentBreakdown;
    if (
      !out.wc2026OpponentBreakdown &&
      poolOppBreakdown &&
      typeof poolOppBreakdown === 'object' &&
      typeof (poolOppBreakdown as Record<string, unknown>).names === 'object' &&
      Object.keys((poolOppBreakdown as { names: Record<string, unknown> }).names).length > 0
    ) {
      recordWcSource('oppBreakdownWc2026', 'supabase-cache', 'player-vs-pool-fallback');
      out.wc2026OpponentBreakdown = poolOppBreakdown;
    }
    out.playerVsPool = {
      ...playerVsPool,
    ...
    if (nameIndex) {
      ...
      if (playerId) {
        ...
        if (cachedStats?.length) {
          ...
          const { totals, onTarget } = buildWorldCupPlayerShotStatsByMatch(
            cachedShots ?? [],
            String(playerId)
          );
          const stats: Array<Record<string, any>> = applyWorldCupPlayerShotsToStatRows(
            cachedStats.map((row) => ({
              ...row,
              source: 'bdl',
        ...
        const wc2026 = (cachedMatches2026 ?? []).filter((m) => m.status === 'completed');
        const fromMatches = await loadCachedPlayerStatsWithFallback({
          playerId: String(playerId),
          matches2026: wc2026,
          apiKey,
        });
        if (fromMatches.length) {
          recordWcSource('playerStatsByName', 'match-detail-fallback', String(playerId));
          return {
            playerMatchStats: fromMatches.map((row) => ({
              ...row,
              source: 'bdl',
              tournament_slug: 'worldcup',
            })),
            matches: summarizeMatches(
              wc2026.filter((m) => fromMatches.some((s) => Number(s.match_id) === m.id))
            ),
          };
        }
      }
...
function readBdlPlayerStat(row: Record<string, unknown>, stat: string, shotsFromEndpoint?: number): number | null {
  ...
  if (stat === 'shots_total') {
    const fromRow =
      bdlStatNum(row.shots_total) ?? bdlStatNum(row.shots) ?? bdlStatNum(row.derived_shots_total);
...
function parseOpponentTeamIdFromRequest(request: NextRequest): {
  ...
  if (!Number.isFinite(opponentTeamId)) return null;
  return {
    opponentTeamId,
    opponentTeamName: request.nextUrl.searchParams.get('opponentTeamName') || '',
    opponentCountryCode: request.nextUrl.searchParams.get('opponentCountryCode'),
  };
}

async function mergeLiveOpponentAllowedIntoBreakdown(
  breakdown: WorldCupPlayerVsOpponentBreakdown | null | undefined,
  opts: {
    opponentTeamId: number;
    opponentTeamName: string;
    opponentCountryCode?: string | null;
...
): Promise<WorldCupPlayerVsOpponentBreakdown | null | undefined> {
  if (!breakdown || !opts.apiKey) return breakdown;
  const slugs = new Set<string>();
  for (const candidate of [opts.opponentCountryCode, opts.opponentTeamName]) {
    const code = resolveWorldCupFlagCode(candidate);
    if (code) slugs.add(code);
  }
  const nameKey = opts.opponentTeamName.trim().toLowerCase();
  if (nameKey) slugs.add(nameKey);
  const hasValues = [...slugs].some((slug) => {
    if ((breakdown.games?.[slug] ?? 0) < 1) return false;
    const goals = breakdown.metrics?.goals?.values?.[slug];
    return typeof goals === 'number' && Number.isFinite(goals);
  });
  if (hasValues) return breakdown;

  const live = await computeBdlWorldCupOpponentAllowedLive(
    opts.opponentTeamId,
    opts.opponentTeamName,
    opts.opponentCountryCode,
    opts.apiKey,
    DEFAULT_SEASON
  );
  if (!live) return breakdown;
  return mergeWorldCupOpponentAllowedSnapshot(
    breakdown,
    live.slug,
    live.name,
    live.games,
...
function playerStatRowRichness(row: Record<string, any>): number {
  const keys = [
    'minutes_played',
    'passes_total',
    'passes',
...
function dashboardPlayerShotsTotal(row: Record<string, any>): number | null {
  return resolveWorldCupPlayerShotsTotalFromRow(row);
}

function mergeDashboardPlayerStatRowFields(
...
function playerStatRowMergeKey(row: Record<string, any>): string {
  const source = String(row.source ?? '').trim().toLowerCase() || 'unknown';
  return `${source}|${row.match_id ?? ''}|${row.team_id ?? ''}`;
}

function mergePlayerStatRowsPreferRich(
  existing: Array<Record<string, any>>,
  incoming: Array<Record<string, any>>
): Array<Record<string, any>> {
  const byKey = new Map<string, Record<string, any>>();
  for (const row of existing) byKey.set(playerStatRowMergeKey(row), row);
  for (const row of incoming) {
    const key = playerStatRowMergeKey(row);
    const prev = byKey.get(key);
    if (!prev) {
      byKey.set(key, row);
    } else if (playerStatRowRichness(row) > playerStatRowRichness(prev)) {
      byKey.set(key, mergeDashboardPlayerStatRowFields(row, prev));
    } else {
      byKey.set(key, mergeDashboardPlayerStatRowFields(prev, row));
    }
  }
  return [...byKey.values()];
}

function bdlPlayerStatRowsLackShotCoverage(rows: Array<Record<string, any>>): boolean {
  const wcRows = rows.filter((row) => {
    const src = String(row.source ?? 'bdl').toLowerCase();
    const slug = String(row.tournament_slug ?? '').toLowerCase();
    return src === 'bdl' || slug === 'worldcup' || slug === 'world-cup' || slug === 'fifa-world-cup';
  });
  if (!wcRows.length) return false;

  let withMinutes = 0;
  let withReliableTotal = 0;
  for (const row of wcRows) {
    const minutes = bdlStatNum(row.minutes_played);
    if (minutes == null || minutes <= 0) continue;
    withMinutes += 1;

    const onTarget = bdlStatNum(row.shots_on_target) ?? 0;
    const rawTotal = Math.max(
      bdlStatNum(row.shots_total) ?? 0,
      bdlStatNum(row.derived_shots_total) ?? 0,
      bdlStatNum(row.shots) ?? 0,
      bdlStatNum(row.total_shots) ?? 0
    );

    // BDL often fills shots_on_target while shots_total stays 0 — repair from match_shots.
    if (onTarget > 0 && rawTotal < onTarget) return true;

    if (rawTotal > 0 && rawTotal >= onTarget) withReliableTotal += 1;
  }
  if (!withMinutes) return false;
  return withReliableTotal / withMinutes < 0.2;
}

async function fetchBdlPlayerShotEvents(
  playerId: string,
  matchIds: number[],
  apiKey: string
): Promise<Array<Record<string, unknown>>> {
  const playerNum = Number(playerId);
  if (!Number.isFinite(playerNum) || !matchIds.length) return [];

  // BDL /match_shots ignores extra match_ids[] when several are sent — fetch per match.
  const uniqueIds = [...new Set(matchIds.filter((id) => Number.isFinite(id)))];
  ...
  for (let offset = 0; offset < uniqueIds.length; offset += concurrency) {
    ...
    const batches = await Promise.all(
      chunk.map(async (matchId) => {
        ...
        params.append('match_ids[]', String(matchId));
        try {
          const shots = await bdlFetchAll<Record<string, unknown>>('/match_shots', params, apiKey, {
            cursor: true,
            maxPages: 6,
          });
          return shots.filter((shot) => Number(shot.player_id) === playerNum);
        } catch {
          return [];
        }
      })
...
function mergePlayerShotEvents(
...
function cachedPlayerShotEventsAreComplete(
  cached: Array<Record<string, unknown>>,
  neededIds: number[],
  playerId: string
): boolean {
  if (!cached.length) return false;
  if (!cached.some((shot) => shot.shot_type != null || shot.on_target != null)) return false;
  const pid = String(playerId);
  const covered = new Set<number>();
  for (const shot of cached) {
    if (String(shot.player_id ?? '') !== pid) continue;
    const matchId = Number(shot.match_id);
    if (Number.isFinite(matchId)) covered.add(matchId);
  }
  return neededIds.every((id) => covered.has(id));
}

async function resolveDashboardPlayerShots(opts: {
  playerId: string;
  matchIds: number[];
  apiKey?: string;
  skipCache?: boolean;
}): Promise<Array<Record<string, unknown>>> {
  const neededIds = [...new Set(opts.matchIds.filter((id) => Number.isFinite(id)))];
  if (!neededIds.length) return [];

  let cached = opts.skipCache
    ? []
    : ((await readWcCache<Array<Record<string, unknown>>>(WC2026.playerShots(opts.playerId))) ?? []);

  const cacheComplete = cachedPlayerShotEventsAreComplete(cached, neededIds, opts.playerId);
  if (opts.apiKey && (opts.skipCache || !cacheComplete)) {
    const fetchIds = opts.skipCache || !cached.length ? neededIds : neededIds.filter((id) => {
      const pid = String(opts.playerId);
      return !cached.some(
        (shot) => String(shot.player_id ?? '') === pid && Number(shot.match_id) === id
      );
    });
    const idsToFetch = fetchIds.length ? fetchIds : neededIds;
    recordWcSource('playerShots', 'bdl-live', opts.playerId);
    const live = await fetchBdlPlayerShotEvents(opts.playerId, idsToFetch, opts.apiKey);
    cached = mergePlayerShotEvents(
      cacheComplete ? cached : cached.filter((shot) => shot.shot_type != null || shot.on_target != null),
      live
    );
    if (cached.length) {
      void setWorldCupCache(WC2026.playerShots(opts.playerId), cached);
    }
  }

  const pid = String(opts.playerId);
  return cached.filter((shot) => {
    const matchId = Number(shot.match_id);
    return neededIds.includes(matchId) && String(shot.player_id ?? '') === pid;
  });
}

function enrichPlayerStatRowsWithShotEvents(
  rows: Array<Record<string, any>>,
  playerShots: Array<Record<string, unknown>>,
  playerId?: string
): Array<Record<string, any>> {
  if (!rows.length) return rows;
  return applyWorldCupPlayerShotsToStatRows(rows, playerShots, playerId);
}

async function enrichDashboardPlayerStatsWithShots(opts: {
  playerId: string;
  playerMatchStats: Array<Record<string, any>>;
  apiKey?: string;
  seasonMatchIds?: Set<number>;
}): Promise<Array<Record<string, any>>> {
  if (!opts.playerMatchStats.length) return opts.playerMatchStats;

  const matchIds = worldCupMatchIdsNeedingPlayerShotEvents(
    opts.playerMatchStats,
    opts.seasonMatchIds
  );
  const lacksCoverage = bdlPlayerStatRowsLackShotCoverage(opts.playerMatchStats);

  let playerShots: Array<Record<string, unknown>> = [];
  if (opts.apiKey && matchIds.length) {
    playerShots = await resolveDashboardPlayerShots({
      playerId: opts.playerId,
      matchIds,
      apiKey: opts.apiKey,
      skipCache: lacksCoverage,
    });
  } else {
    playerShots =
      (await readWcCache<Array<Record<string, unknown>>>(WC2026.playerShots(opts.playerId))) ?? [];
  ...
  return enrichPlayerStatRowsWithShotEvents(opts.playerMatchStats, playerShots, opts.playerId);
}
...
function preserveDashboardPlayerStatRow(row: Record<string, any>): Record<string, any> {
  const out: Record<string, any> = { ...row };
  if (row.source != null && String(row.source).trim()) out.source = row.source;
  else if (!out.source) out.source = 'bdl';
  if (row.tournament_slug != null && String(row.tournament_slug).trim()) {
    out.tournament_slug = row.tournament_slug;
  } else {
    delete out.tournament_slug;
  }
  return out;
}

/** Props-identical intl + club + BDL WC pool for dashboard charts. */
async function mergePlayerCrossSourceWorldCupHistory(opts: {
  playerName: string;
  playerId?: string | null;
  teamId?: string | null;
  nationHint?: string | null;
  playerMatchStats: Array<Record<string, any>>;
  playerMatches: Map<string, Record<string, any>>;
}): Promise<Array<Record<string, any>>> {
  recordWcSource('propsGameLogs', 'merged', opts.playerName);
  const bundle = await loadWorldCupPlayerHistoryBundle(opts.playerName, {
    teamId: opts.teamId,
    nationHint: opts.nationHint,
    playerId: opts.playerId,
  });

  const withPlayerId = (row: Record<string, any>): Record<string, any> => {
    const normalized = preserveDashboardPlayerStatRow(row);
    if (!opts.playerId) return normalized;
    return {
      ...normalized,
      player_id: opts.playerId,
      player: normalized.player ? { ...normalized.player, id: opts.playerId } : { id: opts.playerId },
    };
  };

  const historyRows = (bundle.mergedStatRows as Array<Record<string, any>>).map((row) =>
    withPlayerId({
      ...row,
      match_id: row.match_id ?? row.source_match_id,
    })
  );

  let stats = mergePlayerStatRowsPreferRich(
    historyRows,
    opts.playerMatchStats.map((row) => withPlayerId(row))
  );

  for (const match of bundle.matches as Array<Record<string, any>>) {
    const id = String(match.id ?? match.match_id ?? '');
    if (!id) continue;
    const existing = opts.playerMatches.get(id);
    if (!existing || dashboardMatchRichness(match) >= dashboardMatchRichness(existing)) {
      opts.playerMatches.set(id, match);
    }
  }
...
function playerHasWc2026Appearance(
  ...
  return rows.some((row) => {
    ...
    return [
      ...
      'yellow_cards',
      'red_cards',
    ].some((key) => bdlStatNum(row[key]) != null);
  });
}

function playerWc2026MatchIdsWithAppearance(
  rows: Array<Record<string, any>>,
  completed2026Ids: Set<number>
): Set<number> {
  const ids = new Set<number>();
  for (const row of rows) {
    const matchId = Number(row.match_id ?? row.source_match_id);
    if (!Number.isFinite(matchId) || !completed2026Ids.has(matchId)) continue;
    if (!playerHasWc2026Appearance([row], completed2026Ids)) continue;
    ids.add(matchId);
  }
  return ids;
}

/** True when the team has a newer completed WC2026 game than the player's latest appearance. */
function playerWc2026StatsBehindTeamSchedule(
  rows: Array<Record<string, any>>,
  matches2026: BdlMatch[],
  teamId: number
): boolean {
  const completed2026Ids = wc2026CompletedMatchIdSet(matches2026);
  const appearedMatchIds = playerWc2026MatchIdsWithAppearance(rows, completed2026Ids);
  if (!appearedMatchIds.size) return false;

  const teamMatches = completedMatchesForTeam(matches2026, teamId);
  if (!teamMatches.length) return false;

  const latestTeamMatch = teamMatches[0];
  const latestTeamId = Number(latestTeamMatch.id);
  if (!Number.isFinite(latestTeamId)) return false;
  if (appearedMatchIds.has(latestTeamId)) return false;

  const latestTeamTs = Date.parse(String(latestTeamMatch.datetime ?? '')) || 0;
  const matchById = new Map<number, BdlMatch>();
  for (const match of matches2026) {
    const id = Number(match.id);
    if (Number.isFinite(id)) matchById.set(id, match);
  }

  let playerLatestTs = 0;
  for (const matchId of appearedMatchIds) {
    const match = matchById.get(matchId);
    const row = rows.find((candidate) => Number(candidate.match_id ?? candidate.source_match_id) === matchId);
    const ts =
      Date.parse(
        String(match?.datetime ?? row?.match_date ?? row?.date ?? row?.game_date ?? '')
      ) || 0;
    playerLatestTs = Math.max(playerLatestTs, ts);
  }

  return latestTeamTs > playerLatestTs;
}
...
async function loadPlayerStatsFrom2026MatchDetails(opts: {
  playerId: string;
  matches2026: BdlMatch[];
  maxMatches?: number;
  apiKey?: string;
}): Promise<Array<Record<string, any>>> {
  const completedIds = opts.matches2026
    .filter((match) => match.status === 'completed')
    .map((match) => match.id)
    .filter((id): id is number => Number.isFinite(id))
    .slice(0, opts.maxMatches ?? 64);
  if (!completedIds.length) return [];
...
async function loadCachedPlayerStatsWithFallback(opts: {
  playerId: string;
  teamId?: number | null;
  matches2026?: BdlMatch[];
  apiKey?: string;
  ...
  const cachedStats = await readWcCache<any[]>(WC2026.playerStats(opts.playerId));
  if (cachedStats?.length) {
    recordWcSource('playerStats', 'supabase-cache', opts.playerId);
    playerMatchStats = cachedStats;
  } else {
    recordWcSource('playerStats', 'cache-miss', opts.playerId);
  }
  ...
  const completed2026Ids = wc2026CompletedMatchIdSet(matches2026);
  const teamIdNum =
    opts.teamId != null && Number.isFinite(Number(opts.teamId)) ? Number(opts.teamId) : null;

  if (
    playerMatchStats.length &&
    teamIdNum != null &&
    opts.apiKey &&
    playerWc2026StatsBehindTeamSchedule(playerMatchStats, matches2026, teamIdNum)
  ) {
    const live = await fetchLivePlayerStatsById(opts.playerId, opts.apiKey);
    if (live.length) {
      recordWcSource('playerStats', 'bdl-live-stale-repair', opts.playerId);
      playerMatchStats = mergePlayerStatRowsPreferRich(
        playerMatchStats.map((row) => preserveDashboardPlayerStatRow(row)),
        live.map((row) => preserveDashboardPlayerStatRow(row))
      );
    }
  }

  const needsTeamFallback =
    Boolean(teamIdNum != null) &&
    (!playerMatchStats.length ||
  ...
  if (needsTeamFallback) {
    ...
    if (fromTeam.length) {
      recordWcSource('playerStats', 'match-detail-fallback', String(opts.teamId));
      playerMatchStats = mergePlayerStatRows(playerMatchStats, fromTeam);
    }
  }

  if (!playerMatchStats.length) {
    const fromMatches = await loadPlayerStatsFrom2026MatchDetails({
      playerId: opts.playerId,
      matches2026,
      apiKey: opts.apiKey,
    });
    if (fromMatches.length) {
      recordWcSource('playerStats', 'match-detail-scan', opts.playerId);
      playerMatchStats = mergePlayerStatRows(playerMatchStats, fromMatches);
    }
  }
  ...
  if (!playerMatchStats.length && opts.apiKey) {
    const live = await fetchLivePlayerStatsById(opts.playerId, opts.apiKey);
    if (live.length) {
      recordWcSource('playerStats', 'bdl-live', opts.playerId);
      playerMatchStats = live;
    }
  }

  if (playerMatchStats.length && opts.apiKey && bdlPlayerStatRowsLackShotCoverage(playerMatchStats)) {
    const live = await fetchLivePlayerStatsById(opts.playerId, opts.apiKey);
    if (live.length) {
      recordWcSource('playerStats', 'bdl-live-shot-repair', opts.playerId);
      playerMatchStats = mergePlayerStatRowsPreferRich(
        playerMatchStats.map((row) => preserveDashboardPlayerStatRow(row)),
        live.map((row) => preserveDashboardPlayerStatRow(row))
      );
...
async function refreshDashboardPlayerStats(opts: {
  ...
  const playerMatchStats = await loadCachedPlayerStatsWithFallback({
    playerId: opts.requestedPlayerId,
    teamId:
      opts.requestedTeamId && /^\d+$/.test(opts.requestedTeamId)
        ? Number(opts.requestedTeamId)
        : null,
    matches2026,
    apiKey: opts.apiKey,
  });
  ...
  const normalizedStats = playerMatchStats.map((row) => ({
    ...preserveDashboardPlayerStatRow(row),
    ...
    if (enrich) {
      ...
      if (enrichedCache?.data?.length) {
        return NextResponse.json(
          {
            success: true,
            games: enrichedCache.games,
            data: enrichedCache.data,
            gamesCount: enrichedCache.games.length,
            propsCount: enrichedCache.data.length,
            season: 2026,
            lastUpdated: enrichedCache.lastUpdated,
            nextUpdate: null,
            noWorldCupOdds: false,
            noAflOdds: false,
            ingestMessage: `Fetched ${enrichedCache.data.length} props for ${enrichedCache.games.length} games`,
          },
          { headers: { 'Cache-Control': 'private, no-store' } }
        );
      }
...
async function handleWorldCupPlayerChartOnlyGet(request: NextRequest) {
  const params = request.nextUrl.searchParams;
  const playerName = params.get('playerName')?.trim() || params.get('player')?.trim() || '';
  const requestedPlayerId = params.get('playerId');
  const requestedTeamId = params.get('teamId');
  const requestedTeamName =
    params.get('teamName')?.trim() || params.get('team')?.trim() || null;
  const opponentTeamNameHint =
    params.get('opponentTeamName')?.trim() || params.get('opponent')?.trim() || null;
  ...
  try {
    const bundle = await loadWorldCupPlayerHistoryBundle(playerName, {
      teamId: requestedTeamId,
      nationHint: requestedTeamName,
      playerId: requestedPlayerId,
    });

    const teams = (await readWcCache<BdlTeam[]>(WC2026.teams)) ?? [];
    const matches2026 = (await readWcCache<BdlMatch[]>(WC2026.matches2026)) ?? [];
    const selectedTeam =
      resolveRequestedTeam(teams, requestedTeamId, null) ??
      (requestedTeamName ? findBdlTeamByName(teams, requestedTeamName) : null);
    const featureMatch = findFeatureMatchWithOpponentHint(
      matches2026,
      selectedTeam,
      opponentTeamNameHint
    );

    let playerMatchStats = bundle.mergedStatRows.map((row) => preserveDashboardPlayerStatRow(row));
    if (requestedPlayerId && /^\d+$/.test(requestedPlayerId)) {
      const matches2026Completed = matches2026.filter((match) => match.status === 'completed');
      const liveStats = await loadCachedPlayerStatsWithFallback({
        playerId: requestedPlayerId,
        teamId: selectedTeam?.id ?? null,
        matches2026: matches2026Completed,
        apiKey,
      });
      playerMatchStats = mergePlayerStatRowsPreferRich(
        playerMatchStats,
        liveStats.map((row) => preserveDashboardPlayerStatRow(row))
      );
      playerMatchStats = playerMatchStats.map((row) => ({
        ...row,
        player_id: requestedPlayerId,
        player: row.player
          ? { ...(row.player as Record<string, unknown>), id: requestedPlayerId }
          : { id: requestedPlayerId },
      }));
      playerMatchStats = await enrichDashboardPlayerStatsWithShots({
        playerId: requestedPlayerId,
        playerMatchStats,
        apiKey,
        seasonMatchIds: new Set(
          matches2026Completed.map((match) => Number(match.id)).filter(Number.isFinite)
        ),
      });
    }

    const chartMatches = normalizeDashboardChartMatches(
      bundle.matches as Array<Record<string, any>>
    );
    const liveMatchIds = new Set(
      playerMatchStats.map((row) => String(row.match_id ?? row.source_match_id ?? '')).filter(Boolean)
    );
    const supplementalMatches = matches2026
      .filter((match) => liveMatchIds.has(String(match.id)))
      .map((match) => summarizeMatches([match])[0])
      .filter(Boolean);
    const chartMatchesWithLive = normalizeDashboardChartMatches([
      ...chartMatches,
      ...supplementalMatches,
    ] as Array<Record<string, any>>);
    const playerMatchStatsWithContext = attachStatRowMatchContext(
      playerMatchStats,
      chartMatchesWithLive
    );

    let chartPlayerShots: Array<Record<string, unknown>> = [];
    if (requestedPlayerId && /^\d+$/.test(requestedPlayerId)) {
      const wc2026IdSet = new Set(
        matches2026.filter((match) => match.status === 'completed').map((match) => Number(match.id))
      );
      const chartMatchIds = worldCupMatchIdsNeedingPlayerShotEvents(
        playerMatchStatsWithContext,
        wc2026IdSet
      );
      if (chartMatchIds.length) {
        chartPlayerShots = await resolveDashboardPlayerShots({
          playerId: requestedPlayerId,
          matchIds: chartMatchIds,
          apiKey,
        });
      }
    }
    ...
    return NextResponse.json(
      {
        ...
        teams,
        matches: chartMatchesWithLive,
        playerMatches: chartMatchesWithLive,
        playerMatchStats: playerMatchStatsWithContext,
        playerShots: chartPlayerShots,
        selectedTeam,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to load player chart data' },
      { status: 500 }
    );
  }
}
...
async function handleWorldCupDashboardGet(request: NextRequest) {
  ...
  if (request.nextUrl.searchParams.get('playerChartOnly') === '1') {
    return handleWorldCupPlayerChartOnlyGet(request);
  }
  ...
  if (request.nextUrl.searchParams.get('playerVsPool') === '1') {
    try {
      ...
      if (scope === 'worldcup') {
        const opponentRequest = parseOpponentTeamIdFromRequest(request);
        ...
        if (cached && Array.isArray(cached.players) && (cached.players as unknown[]).length > 0) {
          ...
          let payload = cached as Record<string, unknown>;
          if (opponentRequest && apiKey && payload.opponentBreakdown) {
            const qualifiedUniverse = await ensureWorldCupQualifiedUniverse(apiKey);
            const patched = await mergeLiveOpponentAllowedIntoBreakdown(
              payload.opponentBreakdown as WorldCupPlayerVsOpponentBreakdown,
              { ...opponentRequest, apiKey, qualifiedUniverse }
            );
            if (patched) payload = { ...payload, opponentBreakdown: patched };
          }
        ...
        if (apiKey) {
          ...
          const opponentRequest = parseOpponentTeamIdFromRequest(request);
          if (opponentRequest && payload.opponentBreakdown) {
            const patched = await mergeLiveOpponentAllowedIntoBreakdown(
              payload.opponentBreakdown,
              { ...opponentRequest, apiKey, qualifiedUniverse }
            );
            if (patched) payload.opponentBreakdown = patched;
          }
        }
  ...
  if (request.nextUrl.searchParams.get('oppBreakdown') === '1') {
    try {
      ...
      if (wcOnly) {
        const opponentRequest = parseOpponentTeamIdFromRequest(request);
        const qualifiedUniverse = opponentRequest && apiKey ? await ensureWorldCupQualifiedUniverse(apiKey) : undefined;

        const patchWcOnlyPayload = async (raw: Record<string, unknown>) => {
          if (!opponentRequest || !apiKey || !raw.metrics) return raw;
          const patched = await mergeLiveOpponentAllowedIntoBreakdown(
            raw as WorldCupPlayerVsOpponentBreakdown,
            { ...opponentRequest, apiKey, qualifiedUniverse }
          );
          return patched ? { ...raw, ...patched } : raw;
        };
        ...
        if (
          ...
          const body = await patchWcOnlyPayload(cached);
          return NextResponse.json(
            { ...body, wcOnly: true },
            {
              headers: {
        ...
        if (Object.values(payload.games ?? {}).every((g) => (g ?? 0) < 1)) {
          const poolCached = await readWcCache<{
            opponentBreakdown?: WorldCupPlayerVsOpponentBreakdown;
          }>(WC2026.playerVsPoolWc);
          const poolOb = poolCached?.opponentBreakdown;
          if (poolOb && Object.keys(poolOb.names ?? {}).length > 0) {
            recordWcSource('oppBreakdownWc2026', 'supabase-cache', 'player-vs-pool-fallback');
            payload = {
              window: poolOb.window ?? 0,
              generatedAt: new Date().toISOString(),
              names: poolOb.names,
              games: poolOb.games ?? {},
              totalGames: poolOb.totalGames ?? {},
              metrics: poolOb.metrics,
            };
          }
        }
        ...
        const body = await patchWcOnlyPayload(payload as unknown as Record<string, unknown>);
        return NextResponse.json(
          { ...body, wcOnly: true },
          {
            headers: {
  ...
  try {
      ...
      if (cachedDashboard) {
        ...
        if (requestedPlayerId && /^\d+$/.test(requestedPlayerId)) {
          ...
          const existingStats = Array.isArray(payload.playerMatchStats)
            ...
            : [];
          const payloadMatches = Array.isArray(payload.matches)
            ? (payload.matches as Array<Record<string, any>>)
            : [];
          let completed2026Ids = new Set(
            payloadMatches
              .filter((match) => String(match.status ?? '') === 'completed')
              .map((match) => Number(match.id))
              .filter(Number.isFinite)
          );
          const cachedMatches2026 = await readWcCache<BdlMatch[]>(WC2026.matches2026);
          if (!completed2026Ids.size) {
            completed2026Ids = wc2026CompletedMatchIdSet(cachedMatches2026 ?? []);
          }
          const refreshTeamId =
            requestedTeamId && /^\d+$/.test(requestedTeamId) ? Number(requestedTeamId) : null;
          const statsBehindTeamSchedule =
            refreshTeamId != null &&
            playerWc2026StatsBehindTeamSchedule(
              existingStats,
              cachedMatches2026 ?? [],
              refreshTeamId
            );
          const shouldRefreshPlayerStats =
            request.nextUrl.searchParams.get('refreshPlayerStats') === '1' ||
            existingStats.length === 0 ||
            !playerHasWc2026Appearance(existingStats, completed2026Ids) ||
            bdlPlayerStatRowsLackShotCoverage(existingStats) ||
            statsBehindTeamSchedule;
          ...
          const resolvedPlayerName = await resolveRequestedPlayerNameWithFallback(
            payload,
            requestedPlayerId,
          ...
          if (resolvedPlayerName) {
            try {
              mergedPlayerMatchStats = await mergePlayerCrossSourceWorldCupHistory({
              ...
              console.warn('[world-cup/dashboard] cached player cross-source merge failed:', mergeError);
            }
          } else {
            console.warn('[world-cup/dashboard] missing playerName for historical WC merge', {
              playerId: requestedPlayerId,
            });
          }

          mergedPlayerMatchStats = await enrichDashboardPlayerStatsWithShots({
            playerId: requestedPlayerId,
            playerMatchStats: mergedPlayerMatchStats,
            apiKey,
          });

          const refreshedPlayerShots = await resolveDashboardPlayerShots({
            playerId: requestedPlayerId,
            matchIds: [
              ...new Set(
                mergedPlayerMatchStats.flatMap((row) => statRowMatchIds(row as Record<string, unknown>))
              ),
            ],
            apiKey,
          });
          ...
          payload = {
            ...
            playerShots: refreshedPlayerShots,
          };
        }
    ...
    if (requestedPlayerId && /^\d+$/.test(requestedPlayerId)) {
      playerMatchStats = await loadCachedPlayerStatsWithFallback({
        playerId: requestedPlayerId,
    ...
    // ── Player shots (for numeric playerId mode) — cache-first, BDL live fallback ─
    let playerMatches: BdlMatch[] = [];
    let playerShots: any[] = [];
    if (requestedPlayerId && Array.isArray(playerMatchStats) && playerMatchStats.length) {
      const wc2026IdSet = new Set(matches.map((m) => Number(m.id)).filter(Number.isFinite));
      const neededIds = worldCupMatchIdsNeedingPlayerShotEvents(
        playerMatchStats as Array<Record<string, unknown>>,
        wc2026IdSet
      );
      if (neededIds.length) {
        const mergedLookupMatches = mergeBdlMatches(dashboardAllSeasonMatches, matches);
        playerMatches = mergedLookupMatches.filter((match) => neededIds.includes(Number(match.id)));
      }
      playerShots = await resolveDashboardPlayerShots({
        playerId: requestedPlayerId,
        matchIds: neededIds,
      ...
      if (playerShots.length) {
        recordWcSource('playerShots', 'resolved', requestedPlayerId);
      } else {
    ...
    for (const shot of playerShots) {
      ...
      if (isWorldCupShotOnTarget(shot as Record<string, unknown>)) {
    ...
    const playerRowsWithDerivedShots = (playerMatchStats as Array<Record<string, any>>).map((row) => {
      ...
      for (const matchId of matchIds) {
        const hit = derivedShotsByMatch.get(matchId);
        if (hit) {
          derived = hit;
          break;
        }
      }
      const merged = { ...row, ...derived };
      return applyWorldCupPlayerShotsToStatRows(
        [merged],
    ...
    // player so the main chart shows every game across club + BDL + intl.
    // Keep the full BDL player cache as the base (not 2026-WC-only); wc2026 filter
    // below is only for PvT / team-resolution helpers.
    let mergedPlayerMatchStats: Array<Record<string, any>> = playerRowsWithDerivedShots.map((row) =>
      preserveDashboardPlayerStatRow(row)
    );
    ...
    if (requestedPlayerName) {
      try {
        const playerMatchMap = new Map<string, Record<string, any>>();
        for (const match of mergedPlayerMatches) {
          const id = String(match.id ?? '');
          if (id) playerMatchMap.set(id, match);
        }
        mergedPlayerMatchStats = await mergePlayerCrossSourceWorldCupHistory({
          ...
          nationHint: requestedTeamName,
          playerMatchStats: mergedPlayerMatchStats,
          playerMatches: playerMatchMap,
        });
        mergedPlayerMatches = Array.from(playerMatchMap.values());
      } catch (mergeError) {
