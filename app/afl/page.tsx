...
function aflTopPicksModalLocalIsoDate(value: Date): string {
  return aflTopPicksModalBrisbaneYmd(value);
}

function aflTopPicksModalGrade(
  side: 'OVER' | 'UNDER' | null,
  line: number | null,
  actual: number | null
): AflTopPicksModalPick['result'] {
  if (!side || line == null || actual == null) return '-';
  if (side === 'OVER') return actual > line ? 'Hit' : 'Miss';
  return actual < line ? 'Hit' : 'Miss';
}

function aflTopPicksModalGradePickFromStored(pick: AflTopPicksModalPick, gameDate: string): AflTopPicksModalPick {
  const storedActual =
...
function aflTopPicksModalGroupsFromRecords(records: AflTopPicksHistoryRecord[]): AflTopPicksModalGroup[] {
  return records.map((record) => {
    const gameDate =
      aflTopPicksModalGameDateBrisbane(record.commenceTime) ?? String(record.commenceTime ?? '').slice(0, 10);
    return {
      ...
      picks: record.picks.map((pick) => aflTopPicksModalGradePickFromStored(pick, gameDate)),
    };
  });
}
...
function aflTopPicksModalResultMark(result: AflTopPicksModalPick['result']): string {
...
function aflTopPicksModalMergeCurrentGroups(
  ...
  const filterBlockedPicks = (group: AflTopPicksModalGroup): AflTopPicksModalGroup => {
    const picks = group.picks.filter((pick) => {
      const key = aflTopPicksPlayerSideKey(pick.playerName, pick.recommendedSide);
      return !key || !blocked.has(key);
    });
    return picks.length === group.picks.length ? group : { ...group, picks };
  };

  const groupHasLockedResults = (group: AflTopPicksModalGroup): boolean =>
    group.picks.some((pick) => typeof pick.actualDisposals === 'number' && Number.isFinite(pick.actualDisposals));

  for (const group of latestRoundHistoryGroups) {
    if (group.picks.length > 0) {
      byKey.set(group.gameKey, filterBlockedPicks(group));
    }
  }

  for (const group of liveGroups) {
    if (!aflTopPicksModalAllowsLivePickUpdates(group.commenceTime)) continue;
    if (group.picks.length === 0) continue;
    const existing = byKey.get(group.gameKey);
    const isUpcoming = aflTopPicksModalIsUpcomingGame(group.commenceTime);
    if (existing && isUpcoming && !groupHasLockedResults(existing)) {
  ...
  for (const group of latestRoundHistoryGroups) {
    if (!byKey.has(group.gameKey) && group.picks.length > 0) {
      byKey.set(group.gameKey, filterBlockedPicks(group));
    }
  }
...
function aflTopPicksModalPickShowResult(pick: AflTopPicksModalPick): boolean {
  return pick.result === 'Hit' || pick.result === 'Miss';
}
...
function AflTopPicksModal({
  ...
  resolveTeamLogo: (teamName: string, logoByTeam: Record<string, string>) => string | null;
  formatPastLineDate: (value: string | null | undefined) => string;
}) {
  ...
  const summary = useMemo(() => {
    ...
    return { hits, misses, hitRate: decided > 0 ? (hits / decided) * 100 : 0 };
  }, [historyGroups, isCurrentRound]);

  const loadHistoryRound = useCallback(async (roundKeysToLoad: string[]) => {
    ...
    setHistoryGroups(aflTopPicksModalGroupsFromRecords(records));
  }, []);
    ...
    const load = async () => {
      ...
      try {
        ...
        const resolvedRoundKey =
          calendarAflTopPicksRoundKeyForDate(aflTopPicksModalBrisbaneYmd(new Date())) ??
          calendarAflTopPicksRoundKey(liveRecords[0]?.commenceTime ?? historyRecords[0]?.commenceTime ?? null);
        const scopedLiveRecords = aflTopPicksModalFilterRecordsByRoundWindow(liveRecords, resolvedRoundWindow).filter(
          (record) => !resolvedRoundKey || !record.roundKey || record.roundKey === resolvedRoundKey
        );
        const scopedHistoryRecords = aflTopPicksModalFilterRecordsByRoundWindow(historyRecords, resolvedRoundWindow).filter(
          (record) => !resolvedRoundKey || !record.roundKey || record.roundKey === resolvedRoundKey
        );
  ...
  const canGoNext = roundIndex < roundOptions.length - 1;
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      ...
      >
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={onClose} className="text-xs font-semibold text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200">
            Close
          </button>
        </div>
        ...
        {!isCurrentRound && summary ? (
          <div className="mb-3 text-center text-sm font-semibold text-gray-700 dark:text-gray-200">
            ...
            <span className="text-rose-500">{summary.misses}</span>
            <span className="mx-2 text-gray-400">·</span>
            <span>Hit Rate {summary.hitRate.toFixed(1)}%</span>
          </div>
        ) : null}
        {loading ? (
          ...
          <div className={`max-h-[62vh] overflow-auto custom-scrollbar fade-scrollbar rounded-lg border ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
            <table className="min-w-full text-xs">
              <thead className={`sticky top-0 z-10 ${isDark ? 'bg-[#0b1a2c]' : 'bg-white'}`}>
                <tr className="border-b border-gray-200 dark:border-gray-700 text-left">
                  ...
                  <th className="px-2.5 py-2 font-semibold text-gray-700 dark:text-gray-200">Total</th>
                  <th className="px-2.5 py-2 font-semibold text-gray-700 dark:text-gray-200">Result</th>
...
export default function AFLPage() {
  ...
  return (
    <div className="min-h-screen h-screen max-h-screen bg-gray-50 dark:bg-[#050d1a] transition-colors overflow-y-auto overflow-x-hidden overscroll-contain lg:max-h-none lg:overflow-y-hidden lg:overflow-x-auto">
      ...
      <AflTopPicksModal
        isOpen={showAflTopPicksModal}
        onClose={() => setShowAflTopPicksModal(false)}
        isDark={!!mounted && isDark}
        logoByTeam={logoByTeam}
        resolveTeamLogo={resolveTeamLogo}
        formatPastLineDate={formatPastLineDate}
      />
